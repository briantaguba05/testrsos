...
function Dashboard() {
  ...
  async function getData() {
    await getDoc(doc(firestore, "scheduled_ride", scheduledID)).then((snapshot) => {
      if (snapshot.exists()) {
  ...
  async function update() {
    await updateDoc(
      doc(firestore, "scheduled_ride", scheduledID),
      {
        title: values.title,
              ...
              <CardContent>
                <Grid container spacing={5}>
                  <Grid item xs={12} sm={6}>
                    <TextField fullWidth label='Title' value={values.title != null ? values.title : ''} onChange={(e) => setValues(prevValues => { return { ...prevValues, title: e.target.value } })} required />
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <TextField fullWidth label='Description' value={values.title != null ? values.title : ''} onChange={(e) => setValues(prevValues => { return { ...prevValues, description: e.target.value } })} required />
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <DatePicker
